...
class Tsubasa {
    ...
    loadConfig() {
        ...
        try {
            ...
            console.error("Cannot read config:", error.message);
            return {
    ...
    async callStartAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (startResponse.status === 200 && startResponse.data && startResponse.data.game_data) {
                ...
                return { success: false, error: `Error calling api start` };
            }
        } catch (error) {
            return { success: false, error: `Error calling api start: ${error.message}` };
        }
    }
    ...
    async callDailyRewardAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (dailyRewardResponse.status === 200) {
                return { success: true, message: "Successful daily attendance" };
            } else {
                return { success: false, message: "Today you took attendance" };
            }
        } catch (error) {
            if (error.response && error.response.status === 400) {
                return { success: false, message: "Today you took attendance" };
            }
            return { success: false, message: `Daily attendance error: ${error.message}` };
        }
    }
    ...
    async executeTask(initData, taskId, axiosInstance) {
        ...
        try {
            ...
            this.log(`Error while doing mission ${taskId}: ${error.message}`);
            return false;
        }
    }
    ...
    async checkTaskAchievement(initData, taskId, axiosInstance) {
        ...
        try {
            ...
            this.log(`It's a mistake ${taskId}: ${error.message}`);
            return { success: false };
        }
    }
    ...
    async getCardInfo(initData, axiosInstance) {
        ...
        try {
            ...
            console.log(`Error retrieving card information: ${error.message}`);
            return null;
        }
    }
    ...
    async levelUpCards(initData, totalCoins, axiosInstance) {
        if (!this.config.enableCardUpgrades) {
            console.log("Tag upgrades are disabled in config.");
            return totalCoins;
        }
        ...
        do {
            ...
            if (!cardInfo) {
                console.log("Unable to retrieve card information. Cancel card upgrade!");
                break;
            }
                ...
                if (card.unlocked && updatedTotalCoins >= card.cost && card.cost <= this.config.maxUpgradeCost) {
                    ...
                    try {
                        ...
                        if (levelUpResponse.status === 200) {
                            ...
                            leveledUp = true;
                            this.log(`Upgrade your card ${card.name} (${card.cardId}) go up level ${card.level + 1}. Cost: ${card.cost}, Balance còn: ${updatedTotalCoins}`);
                            break;
                        }
                    } catch (error) {
                        if (error.response && error.response.status === 400 && error.response.data && error.response.data.message === 'Wait for cooldown') {
                            this.log(`It is not yet time for the next upgrade for the card ${card.name} (${card.cardId})`, 'warning');
                            cooldownCards.add(card.cardId);
                        } else {
                            console.log(error);
                            this.log(`Card upgrade error ${card.name} (${card.cardId}): ${error.message}`, 'error');
                        }
                    }
                }
            }
    ...
    async callEnergyRecoveryAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (recoveryResponse.status === 200) {
                ...
                return { success: false, error: `Cannot restore energy` };
            }
        } catch (error) {
    ...
    async tapAndRecover(initData, axiosInstance) {
        ...
        while (continueProcess) {
            ...
            while (currentEnergy > 0) {
                ...
                this.log(`Tap success | Energy remains ${tapResult.energy}/${tapResult.max_energy} | Balance : ${tapResult.total_coins}`, 'success');
                currentEnergy = 0;
                ...
                if (recoveryResult.energy === maxEnergy) {
                    ...
                    this.log(`Successful energy recovery | Current energy: ${currentEnergy}/${maxEnergy}`, 'success');
                } else {
                    this.log(`Insufficient mana regeneration | Current energy: ${recoveryResult.energy}/${maxEnergy}`, 'warning');
                    continueProcess = false;
                    break;
                }
            }
    ...
    async callTapLevelUpAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                return { success: false, error: `Upgrade error tap: ${response.status}` };
            }
        } catch (error) {
            return { success: false, error: `Upgrade error tap: ${error.message}` };
        }
    }
    ...
    async callEnergyLevelUpAPI(initData, axiosInstance) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                return { success: false, error: `Upgrade error energy: ${response.status}` };
            }
        } catch (error) {
            return { success: false, error: `Upgrade error energy: ${error.message}` };
        }
    }
        ...
        if (this.config.enableTapUpgrades) {
            while (tap_level < this.config.maxTapUpgradeLevel && total_coins >= tap_level_up_cost && tap_level_up_cost <= this.config.maxUpgradeCost) {
                ...
                if (tapUpgradeResult.success) {
                    ...
                    this.log(`Tap upgrade successful | Level: ${tap_level} | Cost: ${tap_level_up_cost} | Balance: ${total_coins}`, 'success');
                } else {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                try {
                            ...
                            for (const task of startResult.tasks) {
                                ...
                                if (executeResult) {
                                    ...
                                    if (achievementResult.success) {
                                        this.log(`Do the task ${achievementResult.title} thành công | phần thưởng ${achievementResult.reward}`, 'success');
                                    }
                                }
                            }
                        } else {
                            this.log(`There are no missions available.`, 'warning');
                        }

                        const totalTaps = await this.tapAndRecover(initData, axiosInstance);
                        this.log(`Total number of taps: ${totalTaps}`, 'success');
                    ...
                    this.log(`Account processing error ${i + 1}: ${error.message}`, 'error');
                }
